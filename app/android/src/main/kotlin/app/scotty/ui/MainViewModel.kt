...
import app.scotty.model.AppSettings
import app.scotty.service.NearbyTransferService
import app.scotty.service.NfcBeamMessage
import app.scotty.service.NfcMessageHub
import app.scotty.service.NfcReaderManager
import app.scotty.service.TransferStatus
...
class MainViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private val _receivedFiles = MutableStateFlow<List<app.scotty.service.ReceivedFile>>(emptyList())
    val receivedFiles: StateFlow<List<app.scotty.service.ReceivedFile>> = _receivedFiles.asStateFlow()
            ...
            dataStore.data.collect { prefs ->
                ...
                if (historyRaw.isNotEmpty()) {
                    val historical = historyRaw.split(HISTORY_SEP).mapNotNull { line ->
                        ...
                        if (parts.size >= 5) {
                            app.scotty.service.ReceivedFile(
                                fileName     = parts[0],
                                filePath     = parts[1],
    ...
    private fun persistHistory(files: List<app.scotty.service.ReceivedFile>) {
        viewModelScope.launch {
